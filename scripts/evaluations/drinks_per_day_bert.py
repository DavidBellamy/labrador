...
for rep in range(config["train_config"]["num_reps"]):
    ...
    for j, train_config_i in enumerate(gen_combinations(config["train_config"])):
        ...
        for i, (ktrain_index, kval_index) in tqdm(enumerate(kf.split(y_train))):
            ...
            transformer_mse = np.mean((y_kval - y_kpred) ** 2)
            transformer_mses.append(transformer_mse)
        ...
        if use_wandb:
            ...
            wandb.log({"mse_per_config": np.mean(transformer_mses)})
            run.finish()

        if best_mse is None or np.mean(transformer_mses) < best_mse:
            best_mse = np.mean(transformer_mses)
            best_hps = train_config_i
