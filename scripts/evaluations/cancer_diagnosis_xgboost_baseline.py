...
outfile_name = f"cancer_diagnosis_xgboost_baseline_{time_string}"
    ...
    for fraction in tqdm(np.arange(1, num_subsamples + 1) / num_subsamples):
        ...
        for xgb_config in gen_combinations(xgboost_config):
            ...
            for i, (ktrain_index, kval_index) in enumerate(kf.split(X_train)):
                ...
                xgb_crossentropies.append(categorical_ce)
        ...
        xgb_ce = scce(y_test, y_pred).numpy()
        xgb_f1 = f1_score(one_hot(y_test, num_classes), y_pred).numpy()
