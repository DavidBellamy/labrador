...
names_for_plot = ["BERT", "BERT Ablation", "Labrador", "Labrador Ablation"]
...
names = dict(zip(methods, names_for_plot))

color_palette = {
    "BERT": "#F9D978",
    "BERT Ablation": "#FBE8AC",
    "Labrador": "#D62728",
    "Labrador Ablation": "#E77B7B",
}
...
for k, df in df_list.items():
    df_list[k] = df.query("method in @methods")
    df_list[k]["method"] = df_list[k]["method"].map(names)
...
plt.savefig(
    Path("figures/") / f"finetuning_ablations_{time_string}.png",
    dpi=300,
    bbox_inches="tight",
)
plt.close()
